...
import { useWalletStore } from '@/stores/allet';
...
const payers = ref<Payer[]>([
    { name: 'You', address: walletStore.address, disabled: true }
]);

const addPayer = () => {
    payers.value.push({
        name: 'Payer ' + payers.value.length,
        address: null,
        disabled: false
    });
};
...
watch(payers, () => {
    isValid.value = payers.value.every(signer => signer.name.length >= 1 && signer.address?.length === 42);
}, { deep: true });
...
<template>
    <div class="overlay">
        <div class="form">
            <div v-for="(signer, index) in payers" :key="index" class="signer_card">
                <div class="input">
                    <div class="name">
                        <p>Signer {{ index + 1 }}</p>

                        <button v-if="index == 0" @click="addPayer" class="add_signer_btn">
                            <PlusIcon />
                            <p>Add Signer</p>
                        </button>
            ...
            <div class="threshold">
                ...
                <select v-model="threshold">
                    <option v-for="n in payers.length" :key="n" :value="n">{{ n }}</option>
                </select>
            </div>
        </div>
    </div>
